APP.chooseBoxData = {
    getChoose: function () {
        ...
        // hash1
        var hash1 = [];
        for (let i = 0; i < chooseDataRegion.check.length; i++) {
            if (chooseDataRegion.check[i] === true) {
                hash1.push(1);
            } else {
                hash1.push(0);
            }
        ...
        // hash2
        var hash2 = [];
        for (let i = 0; i < chooseDataProduct.check.length; i++) {
            if (chooseDataProduct.check[i] === true) {
                hash2.push(1);
            } else {
                hash2.push(0);
            }
        }
        var hash = hash1.join('') + hash2.join('');
        location.hash = hash;
